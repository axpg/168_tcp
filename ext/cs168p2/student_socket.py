...
class StudentUSocket(StudentUSocketBase):
    ...
    ## End of Stage 1.2 ##
      ...
      if self.acceptable_seg(seg, payload):
        ...
        while not self.rx_queue.empty():
          ...
          s, pkt = self.rx_queue.pop()
          data = pkt.app[self.rcv.nxt |MINUS| pkt.tcp.seq:]
  ...
  def maybe_send(self):
    ...
    while remaining > 0:
      num_bytes = min(remaining, self.mss)
      payload = self.tx_data[:num_bytes]

      p = self.new_packet(data=payload)
      self.tx(p)
      self.tx_data = self.tx_data[num_bytes:]
